...
########################################################################
#### get reads_ranking.txt from parallel-meta results with gg databases in setA ####
########################################################################
args = commandArgs(trailingOnly=TRUE)
directory_table=args[1]
directory_table2=args[2]

### Get the name from the last taxonomic classification
...
#Complete names incomplete
nums <- grep("^[a-z]",tabla$last_class)
tabla$last_class[nums] <- sapply(as.character(tabla$classification[nums]), function(x){paste(strsplit(x, ";")[[1]][length(strsplit(x, ";")[[1]])-1],strsplit(x, ";")[[1]][length(strsplit(x, ";")[[1]])], sep=" ")})

write.table(tabla,"otus_unicos.txt", quote=F, col.names=T, row.names=F, sep="\t")

### bash
system("cut -f4 otus_unicos.txt | sed 's/ Group//g; s/[_]/ /g; s/Escherichia Shigella/Escherichia/g' > last.class_mod.txt") 
system("sed 's/ <phylum>//; s/ <class>//; s/ <cyanobacteria>//; s/ <bacterium>//; s/ <genus>//; s/ <actinomycete>//; s/ <bacteria>//; s/ <enterobacteria>//; s/ <prokaryotes>//' last.class_mod.txt > last.class_mod2.txt")

tabla <- read.delim("otus_unicos.txt", header=T)
last<- read.delim("last.class_mod2.txt", header=T)
tabla_new<- data.frame(reads_id= tabla$reads_id, classification=tabla$classification, last.class=last$last.class, score=tabla$score) 
