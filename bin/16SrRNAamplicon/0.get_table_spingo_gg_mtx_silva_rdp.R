...
########################################################################
#### get reads_ranking.txt from spingo results with GG databases ####
########################################################################
...
tabla<- read.delim("otus_unicos.txt", header=F)
colnames(tabla) <- c("reads_id","score","classification", "last.class")
tabla$last.class<- as.character(tabla$last.class)

nums<- which(tabla$last.class=="")
if (length(nums)>0){
	tabla2 <- tabla[-nums,]
} else {
	tabla2 <- tabla
...
tabla_new<- data.frame(reads_id= tabla2$reads_id, classification= tabla2$classification, last.class= tabla2$last.class, score= tabla2$score)
tabla_new$last.class<- as.character(tabla_new$last.class)

lista <- c("0319-6A21", "0319-6E2", "0319-6G20", "125ds10", "32-20", "55 st3", "5bav B4", "A55 D21", "A714017", "ABS-6", "Acidimicrobiales koll13", "Acidimicrobiales wb1 P06", "Acidobacteria-5", "Acidobacteria iii1-8", "AEGEAN 185", "AF420338", "Agrobacterium undicola", "AKYG1722", "Alteromonadaceae nsmpVI18", "Aminiphilaceae", "Anaerobaculaceae", "Arctic97B-4", "Ardenscatena", "ASSO-13", "At425 EubF1", "AT-s2-57", "B07 WMSP1", "B46", "BA008", "BD2-6", "BD7-11", "BHI80-139", "Blvii28", "BPC076", "Bulleidia p-1630-c5", "C114", "C20", "C21 c20", "C86", "CCM11a", "CF231", "CFB-26", "CL500-15", "Cryocola poae", "CV90", "D168", "D2", "Dechloromonas fungiphilus", "Dehalobacteriaceae", "DRC31", "DS-100", "DTB120", "E29", "E8", "EC214", "EC94", "Ellin5290", "Ellin5301", "Ellin6513", "Ellin6529", "Endoecteinascidiaceae", "[Entotheonellaceae]", "Erwinia dispersa", "Erysipelotrichaceae p-75-a5", "EtOH8", "[Eubacterium]", "EW055", "FAC87", "FFCH10602", "FFCH7168", "FS117-23B-02", "Fusobacteriaceae u114", "FW68", "G07", "GAL08", "GAL15", "Gemm-6", "Geosporobacter Thermotalea", "GN03", "GN04", "GN06", "GOUTA19", "GOUTA4", "HA73", "Haererehalobacter salaria", "HB118", "HDB SIOH1004", "HDBW-WB69", "HOC21", "HTCC2207", "IIb", "IndB3-24", "J115", "JdFBGBact", "JG30-KF-CM45", "JG37-AG-4", "JH-WHS47", "JL-ETNP-Z39", "JTB36", "K82", "Kazan-3B-09", "KIST-JJY010", "KSA1", "KSB1", "KSB4", "last.class", "LCP-26", "LCP-6", "LD1", "[Leptospirales]", "[Leptospirillaceae]", "MBA08", "MB-A2-108", "MBNT15", "MD2894-B20", "MIZ17", "MIZ46", "ML1228J-1", "MLE1-12", "MND4", "MOB121", "[Mogibacteriaceae]", "MSB-4E2", "MSBL6", "MWH-UniP1", "NB1-i", "NC10 wb1-A12", "Niigata-25", "NKB19 noFP H4", "OCS155", "OM190 agg27", "OP8 2", "OPB41", "OPB72", "OS-K", "OS-L", "PAUC26f", "PAUC34f", "PB19", "PBS-III-9", "PD-UASB-13", "[Pedosphaerales]", "Peptococcaceae rc4-4", "PK29", "PL-11B10", "Pla1", "Pla3", "Pla4", "Pseudoramibacter Eubacterium", "PYR10d3", "R4-45B", "RB046", "RCP1-48", "RF16", "RF39", "RFN20", "RFP12", "S25 1238", "SAW1 B44", "SB-1", "SB-34", "SB-5", "SBR1031 oc28", "SBR1093", "SBRH58", "SC3-41", "SC3-56", "Sc-NB04", "Sediment-1", "SHA-109", "SHA-20", "SHA-31", "SHAS460", "SJA-36", "SJA-88", "SM1D11", "SM2F09", "SMB53", "Spirochaetaceae za29", "Sva0485", "Synergistaceae vadinCA02", "Taxon", "TG3-1", "TG5", "Thermoanaerovibrio", "[Thermobaculaceae]", "Thermovirgaceae", "Thiobacteraceae", "TIBD11", "Tindallia Anoxynatronum", "[Tissierellaceae] ecb11", "[Tissierellaceae] ph2", "Tissierella Soehngenia", "TK17 mle1-48", "TK18", "TPD-58", "Tremblayales", "TTA B6", "Ucn15732", "UD5", "V1B07b93", "VHS-B5-50", "WAL 1855D", "WCHB1-25", "WCHB1-84", "WH1-8", "WM88", "YNPFFP6", "Z20", "ZA3409c", "ZB3","WCHB1-50", "MSBL9", "R18-435")


num <- which(tabla_new$last.class%in%lista)

if (length(num)>0){
	for (i in 1:length(num)){
		numero <- num[i]
		x <- as.character(tabla_new$classification[numero])
		tabla_new$last.class[numero] <- strsplit(x, ";")[[1]][length(strsplit(x, ";")[[1]])-1]
		}
}

lista2 <- c("12-24", "4C0d-2", "AD3", "Ardenscatenaceae", "BSV26", "DA052", "GN03","iii1-8", "KSB1", "LCP-89", "MVP-15", "OP1", "OPB72", "[Paraprevotellaceae]", "SBR1093", "Sediment-1", "SJA-176", "SSW63Au", "Sulfobacillaceae", "TA18", "[Thermobaculales]", "TM7-3", "WCHB1-81", "WS2", "Z20", "ZA3648c", "MD2896-B26")


num <- which(tabla_new$last.class%in%lista2)

if (length(num)>0){
	for (i in 1:length(num)){
		numero <- num[i]
		x <- as.character(tabla_new$classification[numero])
		tabla_new$last.class[numero] <- strsplit(x, ";")[[1]][length(strsplit(x, ";")[[1]])-2]
		}
}
